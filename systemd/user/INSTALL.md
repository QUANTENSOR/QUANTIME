...
| 磁盘守卫 | 每条序列开 batch 前量数据根所在文件系统的剩余空间，低于 `--min-free-gb`（unit 里写的 5，默认也是 5；`0` 关闭）→ 不开新 batch，运行记录 `abort_reason: disk_low`，报告 `coverage: failed` 并写明剩余 / 阈值，退出码 1 |
...
# 确认后执行（磁盘守卫同样生效：--min-free-gb，默认 5）
